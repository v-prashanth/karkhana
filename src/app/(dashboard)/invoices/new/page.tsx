      ...
      <main className="min-h-screen bg-background px-4 pb-12 pt-8 xl:px-8">
        ...
        <div className="mx-auto mt-8 grid w-full max-w-6xl gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <Button onClick={() => window.open(shareUrl, "_blank")} variant="ghost" className="h-12">
            ...
            <Button variant="ghost" className="mt-2 w-full text-muted-foreground" onClick={() => router.push("/home")}>
              Back to Dashboard
            </Button>
          </div>
        </div>
  ...
  return (
    <main className="min-h-screen bg-background pb-24">
      ...
      <form onSubmit={handleSubmit((data) => createInvoice.mutate(data))} className="space-y-6 p-4">
        ...
        {(!selectedContact || total <= 0) && !createInvoice.isPending && (
          <p className="text-xs text-red-400 text-center font-medium">
            {!selectedContact ? "Select a client to continue" : "Add at least one item with qty × rate > 0"}
          </p>
        )}

        <Button type="submit" size="lg" className="w-full text-base font-bold shadow-lg shadow-accent/20" disabled={createInvoice.isPending || !selectedContact || total <= 0}>
