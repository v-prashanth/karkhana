...
export default function JobWorkDetailError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Job Work Challan error:", error);
  }, [error]);
    ...
    <main className="flex min-h-screen flex-col items-center justify-center gap-6 bg-background p-8">
      ...
      <div className="text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-xs">
          {error.message || "Unable to load this challan. The database table may not exist yet — run the CATCHUP migration on your live DB."}
        </p>
      </div>
      <div className="flex gap-3">
        ...
        >
          <ArrowLeft className="h-3.5 w-3.5" /> Back to Job Work
        </Link>
      </div>
    </main>
  );
