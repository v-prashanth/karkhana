...
import { Building2, FileText, ImageIcon, MoonStar, Save, SunMedium, Users, ShieldCheck } from "lucide-react";
    ...
    <main className="min-h-screen bg-background pb-24">
      ...
      <div className="space-y-6 p-4">
          ...
          <Card className="border-border">
            <CardContent className="space-y-4 p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Input value={form.tagline} onChange={(e) => setForm((prev) => ({ ...prev, tagline: e.target.value }))} placeholder="Tagline/Slogan" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <Input type="number" value={form.year_established} onChange={(e) => setForm((prev) => ({ ...prev, year_established: e.target.value }))} placeholder="Year Established (e.g. 1998)" />
                <Input value={form.employee_count} onChange={(e) => setForm((prev) => ({ ...prev, employee_count: e.target.value }))} placeholder="Team Size (e.g. 1-10)" />
              </div>
              <Input 
                value={form.capabilities.join(", ")} 
                onChange={(e) => setForm((prev) => ({ ...prev, capabilities: e.target.value.split(",").map(s => s.trim()).filter(Boolean) }))} 
                placeholder="Capabilities (comma separated, e.g. CNC, Fabrication, Turning)" 
              />
            </CardContent>
          </Card>
        </section>
        ...
        <div className="mt-8 space-y-4 border-t border-border pt-8">
          {organization && (
            <Button variant="outline" className="w-full h-12 border-accent/20 text-accent hover:bg-accent/10" onClick={() => router.push("/settings/audit-logs")}>
              <ShieldCheck className="mr-2 h-4 w-4" /> Enterprise Audit Center
            </Button>
          )}
